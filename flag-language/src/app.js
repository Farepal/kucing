...
const countryList = JSON.stringify(
    btoa(JSON.stringify(Object.keys(countryData)))
);

const isoLookup = Object.fromEntries(
    [...countries].map((name) => [
...
app.get("/switch", (req, res) => {
    ...
    if (country.password) {
        if (req.cookies.password === country.password) {
            ...
            res.status(400).send(
                `error: not authenticated for ${req.query.to}`
            );
            return;
        }
    } else {
...
app.get("/view", (req, res) => {
    ...
    if (country.deny.includes(userISO)) {
        res.status(400).json({
            err: `${req.query.country} has an embargo on your country`,
        });
        return;
    }
